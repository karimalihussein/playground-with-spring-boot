...
import { GlobalLabProvider, useLab } from "./context/GlobalLabContext";
import { MultiplexSection } from "./components/MultiplexSection";
import { HolBlockingDemo } from "./components/HolBlockingDemo";
import { BrowserBridgeExplainer } from "./components/BrowserBridgeExplainer";
import { SerializationWall } from "./components/SerializationWall";
import { ThroughputCharts } from "./components/ThroughputCharts";
import { Http2FrameStrip } from "./components/Http2FrameStrip";
import {
  LatencyBars,
  TcpHud,
  MemoryHud,
} from "./components/MetricsHud";
import { LifecycleBar, NetworkFlow, TermTip } from "./components/EduPrimitives";
...
function AppInner() {
  ...
  const runStream = () => {
    ...
    /* Slow consumer: we intentionally wait before handling each chunk so inbound queue grows — teaches backpressure intuition. */
  ...
  const runConcurrent = async () => {
    ...
    await Promise.all(
      Array.from({ length: n }, () => fetch(`/rest/unary/${c}`).then((r) => r.arrayBuffer())),
    );
      ...
      <header className="border-b border-slate-800/80 bg-slate-950/40 backdrop-blur">
        <div className="mx-auto max-w-[1600px] px-4 py-5">
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-400">
            Feel request/response, unary RPC, server streaming, and WebSocket push. Numbers are toys — the goal is to{" "}
            <em>see</em> framing, streaming, multiplexing, and backpressure. References:{" "}
      ...
      <div className="mx-auto flex max-w-[1600px] flex-col gap-4 px-4 pt-4">
        ...
        <div className="grid gap-4 xl:grid-cols-4">
          <PanelCard title="REST" subtitle="HTTP/1.1 · JSON · request → full body" accent="border-blue-500/40 shadow-[0_0_30px_rgba(91,140,255,0.08)]">
            <BadgeRow labels={["HTTP/1.1", "JSON", "Tomcat/MVC", "Duplex: simplex response"]} highlight={0} />
          ...
          <PanelCard title="gRPC Unary" subtitle="HTTP/2 · Protobuf · one framed message" accent="border-emerald-500/40 shadow-[0_0_30px_rgba(0,200,150,0.08)]">
            <BadgeRow labels={["HTTP/2", "Protobuf", "Unary RPC", "Multiplexed"]} highlight={1} />
          ...
          <PanelCard title="gRPC streaming" subtitle="Server stream · SSE in the browser" accent="border-orange-500/40 shadow-[0_0_30px_rgba(255,140,66,0.1)]">
            <BadgeRow labels={["HTTP/2", "Server stream", "SSE relay", "Flow control"]} highlight={2} />
          ...
          <PanelCard title="WebSocket" subtitle="Duplex · persistent socket" accent="border-fuchsia-500/40 shadow-[0_0_30px_rgba(217,70,239,0.08)]">
            <BadgeRow labels={["WebSocket", "Full-duplex", "Frames", "Not gRPC"]} highlight={0} />
...
function BadgeRow({ labels, highlight }: { labels: string[]; highlight: number }) {
  return (
